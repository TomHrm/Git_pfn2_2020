...
void simulation_run(const size_t *num_individuals_input,
                    const double *prob_divide, size_t generations, FILE *logfp)
{
  size_t num_individuals[2] = {num_individuals_input[0],
                               num_individuals_input[1]};
  ...
  {
    num_individuals[0] =
        simulation_step(population, num_individuals[0], pop_size, prob_divide);
  ...
  if (num_individuals[1] != 0 && num_individuals[0] != 0)
  {
    printf("simulation stopped after %d steps (0:%ld, 1:%ld)\n", k,
           num_individuals[0], num_individuals[1]);
  }
}
