...
typedef std::vector<unsigned char> char_vec;
           ...
           unsigned long file_size)
{
  ...
  assert(file_size > 0);
  _maximum_sequence_length = 0;
  _total_sequence_length = 0;
  ...
  *writeptr++ = '\n';
    ...
    if (firstcol)
    {
      if (*readptr == '>')
      {
        if (this -> header_vector.size() > 0)
        {
          ...
          _total_sequence_length += seqlen;
          if (_maximum_sequence_length < seqlen)
          {
            _maximum_sequence_length = seqlen;
          }
        }
      ...
      } else
      {
        ...
        /* check that we have number of headers and of sequences stored
           is the same */
           //nextfree aus c sollte äquivalent zu size sein
  ...
  if (this -> header_vector.size() > 0)
  {
    ...
    seqlen = (unsigned long) (sequence_end - current_sequence - 1);
    if (seqlen == 0)
...
Multiseq::~Multiseq(void)
{
  if(this -> header_vector.size() > 0)
  {
    this -> header_vector.clear();
    delete &this -> header_vector;
  }
  
  if(this -> sequence_vector.size() > 0)
  {
    this -> sequence_vector.clear();
    delete &this -> sequence_vector;
  }
...
{
  //assert(multiseq != NULL);
  return _maximum_sequence_length;
}
...
unsigned long Multiseq::sequence_length(unsigned long seqnum) const
{
  ...
  {
    return (unsigned long) (this ->sequence_end - this -> sequence_vector[seqnum] - 1);
  }
}
