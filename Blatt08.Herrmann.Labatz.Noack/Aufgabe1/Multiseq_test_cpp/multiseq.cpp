    ...
    if (firstcol)
    {
      if (*readptr == '>')
      {
        if (this->header_vector.size() > 0)
        {
          ...
          const unsigned char *prev_seq = this->sequence_vector.back();
      ...
      } else
      {
        ...
        /* check that we have number of headers and of sequences stored
           is the same */
        assert(this->sequence_vector.size() == this->header_vector.size());
      }
    }
    /* update whether we are in the first column, as we need it for the
       next iteration */
...
unsigned long Multiseq::sequence_length(unsigned long seqnum) const
{
  ...
  {
    assert(this->header_vector[seqnum + 1] > this->sequence_vector[seqnum] - 1);
  ...
  } else
  {
    ...
    return (unsigned long) (this->sequence_end - this->sequence_vector[seqnum] -
                            1);
  }
}
...
void Multiseq::show(unsigned long width) const
{
  ...
  for (seqnum = 0; seqnum < this->num_of_sequences(); seqnum++)
  {
    ...
    } else
    {
      ...
      {
        fwrite(
            currentseq + offset, sizeof *currentseq,
            (width < currentlength - offset) ? width : (currentlength - offset),
            stdout);
        fputc('\n', stdout);
      }
    }
